...
const minimap = graph.addLayer(MiniMapLayer, { location: "bottomRight", width: 180, height: 120 });
if (!(minimap instanceof Layer)) throw new Error("Minimap does not use the consumer's Layer.");
if (minimap.context.graph !== graph) throw new Error("Minimap is connected to a different Graph.");

root.dataset.minimapReady = "true";
